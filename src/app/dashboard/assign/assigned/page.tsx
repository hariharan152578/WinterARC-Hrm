...
import toast from "react-hot-toast";
import gsap from "gsap";
import { Plus, Clock, Layers, CheckCircle2, Filter, MoreHorizontal, X } from "lucide-react";
...
function StatCard({ title, value, color, icon }: any) {
  return (
    <div className={`${color} p-5 md:p-6 rounded-[1.5rem] md:rounded-[2rem] relative flex flex-col justify-between h-32 md:h-36 transition-transform hover:scale-[1.02] shadow-sm`}>
      ...
      <div>
        <span className="text-[10px] md:text-xs font-semibold text-gray-700 block mb-1 uppercase tracking-wider">{title}</span>
...
export default function AssignedTasksPage() {
  ...
  const fetchTasks = async () => { try { const res = await api.get("/requestassign/sent"); setTasks(res.data); } catch { toast.error("Failed loading tasks"); } };
  const fetchSubordinates = async () => { try { const res = await getHierarchyUsers(); setUsers(res.data); } catch { toast.error("Failed loading users"); } };

  useEffect(() => {
    const load = async () => { setLoading(true); await Promise.all([fetchTasks(), fetchSubordinates()]); setLoading(false); };
    load();
  }, []);

  useEffect(() => {
    if (!loading && containerRef.current) {
      gsap.from(".animate-section", { y: 20, opacity: 0, stagger: 0.1, duration: 0.6, ease: "power2.out" });
    }
  }, [loading]);
  ...
  const handleAssignTask = async (e: any) => {
    e.preventDefault();
    if (!formData.title || !formData.description || !formData.assignedTo) { toast.error("Fill required fields"); return; }
    try {
      await api.post("/requestassign", { ...formData, assignedTo: [Number(formData.assignedTo)] });
      toast.success("Task Assigned 🚀");
      setIsModalOpen(false);
  ...
  const getProgress = (status: string) => (status === "ASSIGNED" ? 15 : status === "IN_PROGRESS" ? 60 : 100);

  if (loading) return <div className="h-screen flex items-center justify-center font-bold text-slate-400">Loading...</div>;

  return (
    <div ref={containerRef} className="min-h-screen bg-[#f8f9fa] p-4 md:p-8 space-y-6 md:space-y-8 max-w-[1600px] mx-auto text-slate-900">
      
      {/* HEADER */}
      <div className="animate-section bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 flex flex-col md:flex-row justify-between items-center shadow-sm border border-gray-100 gap-4">
        <div>
          <h2 className="text-[10px] md:text-sm font-bold text-gray-500 uppercase tracking-[0.2em] mb-1">Workspace Overview</h2>
          <h1 className="text-2xl md:text-4xl font-black tracking-tight">Assigned Tasks</h1>
        </div>
        <button onClick={() => setIsModalOpen(true)} className="w-full md:w-auto flex items-center justify-center gap-2 px-8 py-4 bg-black text-white rounded-2xl font-bold hover:bg-gray-800 transition active:scale-95 shadow-xl">
          <Plus size={20} /> New Task
        </button>
      </div>
      ...
      {/* KANBAN BOARD */}
      <div className="flex flex-nowrap lg:grid lg:grid-cols-3 gap-4 md:gap-8 animate-section overflow-x-auto pb-6 snap-x scrollbar-hide">
        {[
          { title: "Assigned", filter: "ASSIGNED", color: "bg-[#E0D7FF]/50", dot: "bg-indigo-400" },
          ...
          <div key={col.title} className={`${col.color} rounded-[2rem] p-5 min-w-[85vw] lg:min-w-0 h-[600px] snap-center flex flex-col`}>
            <div className="flex items-center justify-between mb-6 px-2">
              <div className="flex items-center gap-3"><div className={`w-3 h-3 rounded-full ${col.dot}`}></div><h3 className="font-extrabold uppercase tracking-wider">{col.title}</h3></div>
              <span className="text-[10px] font-black text-gray-500 bg-white px-3 py-1 rounded-lg">{tasks.filter(t => t.status === col.filter).length}</span>
            </div>
            <div className="space-y-4 overflow-y-auto">
              {tasks.filter(t => t.status === col.filter).map(task => (
                <div key={task.id} className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition">
                  <span className="text-[9px] text-gray-300 font-bold uppercase">TASK #{task.id}</span>
                  <h4 className="font-bold text-gray-900 mt-1 mb-2">{task.title}</h4>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-[500px] p-8 rounded-[2rem] shadow-2xl relative">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-6 right-6 text-gray-400"><X size={20}/></button>
            <h2 className="text-2xl font-black mb-6">Assign New Task</h2>
            <form onSubmit={handleAssignTask} className="space-y-4">
              <input placeholder="Task Title" className="w-full p-4 bg-gray-50 rounded-xl" onChange={(e)=>setFormData({...formData, title: e.target.value})} />
              <textarea placeholder="Description" rows={3} className="w-full p-4 bg-gray-50 rounded-xl" onChange={(e)=>setFormData({...formData, description: e.target.value})} />
              <select className="w-full p-4 bg-gray-50 rounded-xl" onChange={(e)=>setFormData({...formData, assignedTo: e.target.value})}>
                <option value="">Select Member</option>
                {getAssignableUsers().map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
              </select>
              <button className="w-full bg-black text-white p-4 rounded-xl font-bold">Assign Task</button>
            </form>
          </div>
        </div>
      )}
